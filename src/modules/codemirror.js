...
export function foldOnIndentLvl(view, indentLevel) {
  ...
  for (let line = 1; line < numberOfLines; line++) {
      ...
      if (foldingRange) { foldingRanges.push(foldingRange) }
  }
  ...
  for (const foldingRange of foldingRanges) {
      ...
      if (!lineIntendation || lineIntendation !== indentLevel) { continue }
